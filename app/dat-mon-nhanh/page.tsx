...
export default function DatMonNhanhPage() {
  ...
  async function fetchInitialData() {
    const [
      ...
      supabase
        .from("products")
        .select("id,name,slug,price,badge,image_url,description,is_sold_out,category")
        .eq("is_active", true)
  ...
  const categories = useMemo(() => {
    const list = products
      .map((item) => item.category || "Món ngon")
      .filter(Boolean);
  ...
  return (
    <main className="min-h-screen bg-[#F5FFF8] pb-32">
      {toast && (
      ...
      <section id="cart-section" className="mx-auto mt-8 max-w-6xl px-4">
        <div className="rounded-[32px] bg-white p-5 shadow-xl shadow-neutral-950/5">
          <h2 className="text-2xl font-black text-[#06113C]">Giỏ hàng</h2>
                ...
                >
                  ...
                  <div className="flex-1">
                    ...
                    <p className="mt-2 font-black text-[#00B14F]">
                      {(item.itemTotal * item.quantity).toLocaleString(
                        "vi-VN"
                      )}
                      đ
                    </p>
          ...
          <div className="mt-6 grid gap-3 md:grid-cols-2">
            ...
            <input
              ...
              className="rounded-2xl border border-black/10 px-4 py-4 font-bold md:col-span-2"
            />
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 border-t border-black/5 bg-white p-4 shadow-2xl">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-4">
          <div>
            <p className="text-xs font-bold text-neutral-500">
              {cart.length} món · Ship {shippingFee.toLocaleString("vi-VN")}đ
            </p>
          ...
          <button
            ...
            className="rounded-2xl bg-[#00B14F] px-6 py-4 text-sm font-black text-white disabled:opacity-50"
          >
